...
const adminPrivacyPolicy = require("../models/privacy_policy");
...
const editData = async (req, res) => {
    try {
        ...
        let updateData = await adminPrivacyPolicy.findOneAndUpdate({ admin_id: adminId }, req.body, { new: true });

        res.status(200).json({
            success: true,
            message: "Privacy Pollicy Updated Successfully",
            data: updateData
        });
    } catch (error) {
        console.error(error);
...
const displayData = async (req, res) => {
    try {
        ...
        let displayData = await adminPrivacyPolicy.find();
        if (!displayData) {
            return res.status(400).json({
                success: false,
                message: "Privacy Pollicy Not Found!"
            })
        }
        res.status(200).json({
            success: true,
            message: "Privacy Pollicy Find Successfully",
            data: displayData
        })
    } catch (error) {
        console.error(error);
